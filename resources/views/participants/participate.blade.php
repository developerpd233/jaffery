    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <?php
                    // dd($contest);
                    // dd($participant);
                    // dd(participantExist($contest->id));
                    // dd(participantExist($contest->id) && $contest->status == "1");
                    ?>
                ...
                @csrf
                    ...
                    <div class="main_partispnt">
                        ...
                        <div class="uploder">
                            ...
                            @if($contest->type->slug == 'video')
                                <div class="uploder-img thumbnail feat-img abc">
                                    <h3>Video Thumbnail</h3>
                                    <div class="img">
                                        <div class="input-field">
                                            <div class="input-thumbnail" style="padding-top: .5rem;"></div>
                                        </div>
                                        <!-- <img src="{{ url('img/Group05185.png') }}">
                                        <input type="file" name="video_thumbnail" accept="image/*" required> -->
                         ...
                         <input type="hidden" name="video_record" id="video_record" value="">
...
<div id="video-section" class="video-sec d-none">
    <div class="container-fluid">
        <div class="left">
            <h2>Preview</h2>
        ...
        <div class="right">
            ...
            <button id="upload_video" class="button">Upload Video</button>
        </div>
        <div class="bottom">
            <pre id="log"></pre>
        </div>
...
$(document).ready(function() {
    ...
    {
        var length = $(".uploadimg1 .uploaded .uploaded-image").length;
        var thumbnail_length = $(".thumbnail .uploaded .uploaded-image").length;

        if (length == 0) {
            alert('Featured image is required!');
            e.preventDefault(e);
        }
        if ($(".thumbnail").length > 0 && thumbnail_length == 0) {
...
if ($('#video-section').length > 0) {}
    ...
    $('#upload_video').click(function(e){
        ...
        $.ajax({
          ...
          type: "POST",
          data: fd, //$('#video_record').val(),
          processData: false,
          contentType: false,
          success: function(response) {
            if(response.code == 200) {
              console.log(response);
              $('#video_record').val(response.data);
              $("#videoUpload").attr('src','{{ url("/storage") }}'+response.data);
...
<script>
    jQuery(document).ready(function(){
        jQuery('.input-thumbnail').imageUploader(
        {
            //preloaded: preloaded,
            imagesInputName: 'video_thumbnail',
